 ...
 * {@link IntegerStorageChange}. This observer is removed from the list of
 * active observers when it is called the specified number of times.
 * 
...
public class DoubleValue implements IntegerStorageObserver {
	...
	public void valueChanged(final IntegerStorageChange storage) {
		if (callsLeft > 0) {
			System.out.println(String.format("Double value %d", storage.getNewValue() * 2));

			callsLeft--;
		} else {
			storage.getIntegerStorage().removeObserver(this);
		}
	}
